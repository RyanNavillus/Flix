...
class FlixAPIManager: NSObject {
    ...
    func getVideos(id: Int, callback: @escaping (_ key: String) -> Void) {
        ...
        let task = session.dataTask(with: request) { (data, _, error) in
            ...
            if let error = error {
                ...
                if let videoMaps = map["results"] as? [Any] {
                    if let videoMap = videoMaps.first as? [String: Any],
                        let key = videoMap["key"] as? String{
                            callback(key)
                    }
                }
            }
